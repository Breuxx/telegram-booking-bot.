...
async def add_employee(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text(f"✅ Сотрудник {name} добавлен! Попросите его отправить любое сообщение боту")
        
    except Exception as e:
        logger.error(f"Ошибка при добавлении сотрудника: {str(e)}")
        await update.message.reply_text("⚠️ Неверный формат команды")
...
async def update_employee(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        _, old_name, new_name = update.message.text.split(' ', 2)
